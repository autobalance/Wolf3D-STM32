...
//
    ...
    compseg=(byte *) 0x30000000;//malloc(chunkcomplen);
...
//
    ...
    {
        source = (int32_t *) 0x30000000;//malloc(compressed);
...
//
    ...
    bigbufferseg=(memptr) 0x30000000;//malloc(compressed);
...
// Sprites need to have shifts made and various other junk
//
    byte *pic = (byte *) (0x30000000 + 64000);//malloc(64000);
        ...
        for(int x = 0, scx = 0; x < 320; x++, scx += scaleFactor)
        {
            ...
            for(unsigned i = 0; i < scaleFactor; i++)
                for(unsigned j = 0; j < scaleFactor; j++)
                    vbuf[(scy + i) * bufferPitch + scx + j] = col;
        }
    }
    //free(pic);
    //free(bigbufferseg);
...
//
    ...
    for (plane = 0; plane<MAPPLANES; plane++)
    {
        ...
        expanded = *source;
        source++;
        buffer2seg = (word *) (0x30000000 + expanded);//malloc(expanded);
